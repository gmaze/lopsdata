![](https://www.umr-lops.fr/var/storage/images/_aliases/logo_main/medias-ifremer/medias-lops/logos/logo-lops-2/1459683-4-fre-FR/Logo-LOPS-2.png) 

# LOPS-DATA team mini-camps on data mining

The [Laboratory for Ocean Physics and Satellite remote sensing (LOPS)](https://www.umr-lops.fr/) has a transverse working group on "data". As part of our scientific animation, we organise mini-camps (2h00 long) to get team members started with data mining tools. This repo. hosts all the material presented.

[![badge](https://img.shields.io/badge/launch-Pangeo%20binder-579ACA.svg?logo=data:image/png;base64,)](https://binder.pangeo.io/v2/gh/umr-lops/lopsdata/master)



## Coming-up next:

### Predictive modelling with analogs 
Apr. 24th, 2019, F. Sevellec

## Previously:

### Interactive visualisation tools
Mar. 1st, 2019, A. Grouazel

### Scattering Transform
Dec. 17th, 2018, J.M. Delouis

### Clustering
Oct. 4th, 2018, G. Maze

### Optimal Interpolation
Jun. 29th, 2017, N. Kolodziejczyk, C. De Boyer Montegut, E. Autret

