...
%[nc,ax] = cbar(ax,ext,units,clim,nc,fs,eq,txtplace);
...
%     hold on;plot([1 2],[2 2],'color',[.99 .99 .99]);
else
...
for i=1:1:nc,
   ...
   if vert,
      ...
      tleg(i)=text(2.5,ix,num2str(xtextval(i)),'FontSize',fs,'Horizo','left');
   else
      tleg(i)=text(ix,0.8,num2str(xtextval(i)),'FontSize',fs,'Horizo','center','Vertical','top');
   end;
   ...
   if vert
     ...
     tlx(i)=tl(3);
      ...
      if i==nc, text(i+1,0.8,units,'FontSize',fs,'Horizo','left','Vertical','top');end;
   end
end;
if vert,
%   tlx=max(tlx)*1.2+2;
