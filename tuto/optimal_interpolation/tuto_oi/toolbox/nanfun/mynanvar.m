...
if isequal(w,0) || isequal(w,1)
    ...
    x0 = x - repmat(mynanmean(x, dim), tile);
    y = mynansum(abs(x0).^2, dim) ./ denom; % abs guarantees a real result
...
elseif ~(isvector(w) && all(w(~isnan(w)) >= 0))
    ...
    % Count up non-NaNs.
    n = mynansum(~isnan(x).*w,dim);

    x0 = x - repmat(mynansum(w.*x, dim) ./ n, tile);
    y = mynansum(w .* abs(x0).^2, dim) ./ n; % abs guarantees a real result
end
